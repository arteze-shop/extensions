...
import { ZiinaConfigsList } from "@/modules/ui/ziina-configs/ziina-configs-list";

export const ChannelConfigSection = () => {
  const { data, error, refetch } = trpcClient.appConfig.getZiinaConfigsList.useQuery();
  ...
  if (data && data.length > 0) {
    return <ZiinaConfigsList configs={data} />;
  }
