...
import { ZiinaProblemReporter } from "@/modules/app-problems/ziina-problem-reporter";
...
// todo test
export class RemoveZiinaConfigTrpcHandler {
  baseProcedure = protectedClientProcedure;

  private readonly webhookManager = new ZiinaWebhookManager();
  private readonly logger = createLogger("RemoveZiinaConfigTrpcHandler");
  ...
  private async getRootConfigOrThrow({
    ...
    if (rootConfig.isErr()) {
      throw new TRPCError({
        ...
        message: "Failed to remove Ziina configuration. Please try again.",
      });
    }
  ...
  getTrpcProcedure() {
    return this.baseProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const configToRemove = rootConfig.ziinaConfigsById[input.configId];
        ...
        const webhookRemovingResult = await this.webhookManager.removeWebhook({
          accessToken: configToRemove.accessToken,
        });

        if (webhookRemovingResult.isErr()) {
          /**
           * Ignore - it's possible webhook was removed in Ziina, or in previous call. We can't transactional remove both webhook and our config,
           * so we must accept that inconsistency and document it
           */
          this.logger.warn("Failed to remove Ziina webhook, continuing with config removal", {
            error: webhookRemovingResult.error,
          });
        }
        ...
        const reporter = new ZiinaProblemReporter(
