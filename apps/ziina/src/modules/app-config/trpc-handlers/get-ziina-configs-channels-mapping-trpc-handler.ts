...
// todo test
export class GetZiinaConfigsChannelsMappingTrpcHandler {
  baseProcedure = protectedClientProcedure;

  getTrpcProcedure() {
    return this.baseProcedure.query(
      async ({ ctx }): Promise<Record<string, ZiinaFrontendConfigSerializedFields>> => {
        ...
        const allConfigs = config.value.ziinaConfigsById;
        ...
        return Object.entries(mapping).reduce(
          (acc, [channelID, configId]) => {
            ...
            if (config) {
              acc[channelID] = ZiinaFrontendConfig.createFromZiinaConfig(config);
            }

            return acc;
          },
          {} as Record<string, ZiinaFrontendConfigSerializedFields>,
        );
      },
    );
  }
