...
const getTestCaller = () => {
  ...
  const instance = new RemoveZiinaConfigTrpcHandler({
    webhookManager,
  });
...
const getMockedRootConfig = () =>
  new AppRootConfig(
    ...
    {
      [mockedConfigurationId]: mockedZiinaConfig,
    },
  );

describe("RemoveZiinaConfigTrpcHandler", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("Continue to remove config even if removing webhook fails", async () => {
    ...
    vi.mocked(webhookManager.removeWebhook).mockImplementationOnce(async () =>
      err(new ZiinaWebhookManagerError("Test error")),
    );
  ...
  it("Returns internal server error if config failed to update: ziina config unit", async () => {
    const { caller, mockedAppConfigRepo } = getTestCaller();
    ...
    await expect(
      ...
      `[TRPCError: Failed to remove Ziina configuration. Please try again.]`,
    );
  });
});
