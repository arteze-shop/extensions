...
describe("SaleorMoney", () => {
  describe("createFromZiina", () => {
    it("creates a valid money object for valid input", () => {
      const result = SaleorMoney.createFromZiina({ amount: 1000, currency: "usd" });
    ...
    it("rejects negative amounts", () => {
      const result = SaleorMoney.createFromZiina({ amount: -100, currency: "usd" });
    ...
    it("rejects invalid currency code length", () => {
      const result = SaleorMoney.createFromZiina({ amount: 100, currency: "usdd" });
    ...
    it("rejects unsupported currency codes", () => {
      const result = SaleorMoney.createFromZiina({ amount: 100, currency: "abc" });
  ...
  describe("amount", () => {
    it("handles 0-digit currencies", () => {
      const money = SaleorMoney.createFromZiina({
        amount: 2137,
        currency: "jpy",
      })._unsafeUnwrap();
