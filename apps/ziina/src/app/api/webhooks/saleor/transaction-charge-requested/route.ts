...
import { createZiinaClient } from "@/modules/ziina/ziina-client";
...
const useCase = new TransactionChargeRequestedUseCase({
  ...
  ziinaClientFactory: { create: createZiinaClient },
});
...
const handler = transactionChargeRequestedWebhookDefinition.createHandler(
  withRecipientVerification(async (_req, ctx) => {
    try {
      ...
      const problemReporter = createZiinaProblemReporter(ctx.authData);
      ...
      return result.match(
        (result) => {
          logger.info("Successfully processed webhook request", {
            ...
            ziinaEnv: result.appContext.ziinaEnv,
