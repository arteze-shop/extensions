...
describe("BrokenAppResponse", () => {
  it("getResponse() returns valid Response with status 500 and message with error reason", async () => {
    const getConfigResponse = new BrokenAppResponse({ ziinaEnv: "LIVE" }, new Error("Inner error"));
...
describe("Passing error details to the response when Ziina env is TEST", () => {
  it.each([
    BrokenAppResponse,
    MalformedRequestResponse,
    UnhandledErrorResponse,
    ...
    const response = new ErrorResponse({ ziinaEnv: "TEST" }, error);
